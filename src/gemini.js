...
function renderGeminiControlPanel() {
  ...
  document.getElementById('bd-toggle-select').onclick = () => {
    const isSelectAll = document.getElementById('bd-toggle-select').textContent === 'Select All'
    ...
    document.querySelectorAll('.bulk-delete-checkbox').forEach((c) => (c.checked = isSelectAll))
  }
...
async function runGeminiBulkDelete() {
  ...
  for (const cb of selected) {
    ...
    try {
      const menuBtn =
        chatLink.parentNode.querySelector('button[aria-haspopup="menu"]') ||
        chatLink.parentNode.querySelector('button[data-test-id*="menu"]') ||
        chatLink.parentNode.querySelector('button:last-of-type')

      if (!menuBtn) {
        console.warn('Menu button not found for chat', chatLink)
        return
      }
      menuBtn.click()
      await new Promise((r) => setTimeout(r, 150))

      const btnDelete = document.querySelector('button[data-test-id="delete-button"]')
      if (!btnDelete) {
        console.warn('Delete button not found in menu for chat', chatLink)
        return
      }
      btnDelete.click()
      await new Promise((r) => setTimeout(r, 150))
